...
class AutofillSavingCredentialsDialogFragment : BottomSheetDialogFragment(), CredentialSavePickerDialog {
    ...
    override fun onCancel(dialog: DialogInterface) {
        Timber.v("onCancel: AutofillSavingCredentialsDialogFragment. User decline to save credentials")

        appCoroutineScope.launch {
            val shouldPromptToDisableAutofill = autofillDeclineCounter.userDeclinedToSaveCredentials(getOriginalUrl().extractDomain())

            if (shouldPromptToDisableAutofill) {
                val result = Bundle().also {
                    it.putString(CredentialSavePickerDialog.KEY_URL, getOriginalUrl())
                }
                parentFragment?.setFragmentResult(CredentialSavePickerDialog.resultKeyUserDeclinedToSaveCredentials(getTabId()), result)
            }
        }
    }
